...
func newTestEnv(t *testing.T, pool *pgxpool.Pool, encrypter *crypt.Encrypter) testEnv {
	...
	New(pool, Config{Encrypter: encrypter, Logger: slog.New(slog.DiscardHandler)}).Register(authService.Admin(api, "/admin"))
...
func (e testEnv) signUp(t *testing.T, admin bool) string {
	...
	response := e.api.PostCtx(t.Context(), "/auth/register", map[string]string{"name": "Test", "email": email, "password": testPassword})
...
func testAuthorization(t *testing.T, pool *pgxpool.Pool) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			requireStatus(t, env.api.GetCtx(t.Context(), "/admin/storage-backends", tc.cookie...), tc.status)
			...
			if tc.status != http.StatusOK {
				requireStatus(t, env.api.PostCtx(t.Context(), "/admin/storage-backends", create...), tc.status)
			}
		})
	}
}
...
func testLifecycle(t *testing.T, pool *pgxpool.Pool) {
	...
	response := env.api.PostCtx(ctx, "/admin/storage-backends", env.admin,
	...
	response = env.api.PostCtx(ctx, "/admin/storage-backends", env.admin, newS3Body("Archive", "AKIAEXAMPLE", testSecret))
	requireStatus(t, response, http.StatusCreated)
	archive := decode[Backend](t, response)

	response = env.api.GetCtx(ctx, "/admin/storage-backends", env.admin)
	...
	response = env.api.PutCtx(ctx, "/admin/storage-backends/"+local.ID, env.admin,
	...
	response = env.api.GetCtx(ctx, "/admin/storage-backends/"+local.ID, env.admin)
	requireStatus(t, response, http.StatusOK)
	if got := decode[Backend](t, response); got.Enabled || got.Local.Root != "/data" {
		t.Fatalf("update was not stored: %+v", got)
	}

	response = env.api.PutCtx(ctx, "/admin/storage-backends/"+local.ID, env.admin, s3Body("Local disk", "AKIAEXAMPLE", testSecret))
	requireStatus(t, response, http.StatusUnprocessableEntity)

	requireStatus(t, env.api.DeleteCtx(ctx, "/admin/storage-backends/"+local.ID, env.admin), http.StatusNoContent)
	requireStatus(t, env.api.DeleteCtx(ctx, "/admin/storage-backends/"+local.ID, env.admin), http.StatusNotFound)
	requireStatus(t, env.api.GetCtx(ctx, "/admin/storage-backends/"+local.ID, env.admin), http.StatusNotFound)
	requireStatus(t, env.api.PutCtx(ctx, "/admin/storage-backends/"+local.ID, env.admin,
		map[string]any{"name": "Gone", "enabled": true, "local": map[string]string{"root": "/data"}}), http.StatusNotFound)
	requireStatus(t, env.api.DeleteCtx(ctx, "/admin/storage-backends/"+archive.ID, env.admin), http.StatusNoContent)
}
...
func testCredentials(t *testing.T, pool *pgxpool.Pool) {
	...
	response := env.api.PostCtx(ctx, "/admin/storage-backends", env.admin, newS3Body("Credentials", "AKIAEXAMPLE", testSecret))
	requireStatus(t, response, http.StatusCreated)
	...
	path := "/admin/storage-backends/" + backend.ID
...
func testValidation(t *testing.T, pool *pgxpool.Pool) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			response := env.api.PostCtx(t.Context(), "/admin/storage-backends", env.admin, tc.body)
			requireStatus(t, response, http.StatusUnprocessableEntity)
...
func testMigrationRollback(t *testing.T, pool *pgxpool.Pool) {
	...
	if result.Source.Version != 3 {
		t.Fatalf("rolled back migration %d, want 3", result.Source.Version)
	}
...
func testConnectionChecks(t *testing.T, pool *pgxpool.Pool, server testutil.S3) {
	...
	requireStatus(t, response, http.StatusCreated)
	local := decode[Backend](t, response)
	requireCheck(env.api.PostCtx(ctx, "/admin/storage-backends/"+local.ID+"/check", env.admin), true, works)
	requireStatus(t, env.api.PostCtx(ctx, "/admin/storage-backends/"+local.ID+"/check", env.user), http.StatusForbidden)
	requireStatus(t, env.api.PostCtx(ctx, "/admin/storage-backends/stb_01K4W9T5V8QK3M7ZB0YHXC2FNE/check", env.admin), http.StatusNotFound)

	requireCheck(env.api.PostCtx(ctx, "/admin/storage-backends/check", env.admin,
	...
	requireCheck(env.api.PostCtx(ctx, "/admin/storage-backends/"+bucket.ID+"/check", env.admin), true, works)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			response := env.api.PostCtx(ctx, "/admin/storage-backends/check", env.admin, tc.body)
			if tc.status == http.StatusOK {
	...
	response = env.api.PostCtx(ctx, "/admin/storage-backends/check", env.admin,
	...
	lost := newTestEnv(t, pool, newEncrypter(t))
	requireCheck(lost.api.PostCtx(ctx, "/admin/storage-backends/"+bucket.ID+"/check", lost.admin), false, undecryptableCredentials)
}
